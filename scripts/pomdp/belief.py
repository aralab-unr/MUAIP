...
class MosOOBelief(pomdp_py.OOBelief):
    ...
    def random(self, **kwargs):
        # print(pomdp_py.OOBelief.random(self))
...
def initialize_belief(
    ...
    if representation == "histogram":
        return _initialize_histogram_belief(
            tunnel_states, robot_id, object_ids, prior, robot_orientations
        )
...
def _initialize_histogram_belief(tunnel_states, robot_id, object_ids, prior, robot_orientations):
    """
    ...
    for objid in object_ids:
        ...
        if objid in prior:
            ...
            for pose in tunnel_states:
                state = ObjectState(objid, "target", tuple(pose))
                hist[state] = 1.0
                total_prob += hist[state]
