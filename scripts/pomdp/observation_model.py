...
class MosObservationModel(pomdp_py.OOObservationModel):
    ...
    def __init__(self, min_max_dim, sensor, object_ids, sigma=0.01, epsilon=1):
        self.sigma = sigma
        self.epsilon = epsilon
        observation_models = {
            objid: ObjectObservationModel(
                objid, sensor, min_max_dim, sigma=sigma, epsilon=epsilon
            )
...
class ObjectObservationModel(pomdp_py.ObservationModel):
    def __init__(self, objid, sensor, min_max_dim, sigma=0, epsilon=1):
        """
    ...
    def probability(self, observation, next_state, action, **kwargs):
        ...
        if zi == ObjectObservation.NULL:
            ...
            gaussian = pomdp_py.Gaussian(
               ...
               # [[self.sigma**2, 0, 0], [0, self.sigma**2, 0], [0, 0, self.sigma**2]]#
            )
    ...
    def _sample_zi(self, event, next_state, argmax=False):
        if event == "A":
            ...
            gaussian = pomdp_py.Gaussian(
                list(object_true_pose), [[self.sigma**2, 0], [0, self.sigma**2]]#[[self.sigma**2, 0, 0],                                 [0, self.sigma**2, 0],                                 [0, 0, self.sigma**2]]#
            )
            if not argmax:
                zi = gaussian.random()
            else:
            ...
            zi = (
                random.uniform(self._min_max_dim[0][0], self._min_max_dim[0][1]),  # x axis
                random.uniform(self._min_max_dim[1][0], self._min_max_dim[1][0])
            )  # y axis
        else:  # event == C
